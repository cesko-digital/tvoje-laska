...
import { getFriends } from "app/api/member/member";
...
const ProfileFriends = async () => {
  ...
  const members = await getFriends();
      ...
      {members.map((member: any) => {
        ...
        return (
          <UserCard
            ...
            photo={
              member.avatar_urls?.full.slice(0, 5) === "https"
                ? member.avatar_urls?.full
                : "https:" + member.avatar_urls?.full
            }
            userIsActive={false}
            key={member.id}
          />
  ...
  return (
    <>
      <div className="flex justify-center items-center flex-col gap-4">
        <Tabs request={request} favourite={favourite} />
      </div>
      <div className="flex justify-center py-4">
        <CardContainer
          variant="default"
          padding="smaller"
          className="w-[358px] h-[204px] bg-mustard-10 bg-[url('/assets/images/ellipse.svg')] bg-right-top bg-no-repeat"
        >
